...
public class TracksFigure extends Figure { // Figure { // PlotBoxColumn {
   ...
   *           the exception
   */
  public void setTracks(TrackTree tracks, String genome, PlotStyle style, TitleProperties titlePosition)
      throws Exception {
    mTracks = tracks;
    ...
    for (TreeNode<Track> node : tracks) {
      ...
      axes.getX1Axis().getTicks().getMajorTicks().getFontStyle()
      ...
      for (TreeNode<Track> child : children) {
        TrackSubFigure subFigure = child.getValue().createGraph(genome, titlePosition);
   ...
   * 
   * // Update a new plot subFigure = track.getValue().updateGraph(displayRegion,
   * resolution, normalize, width, height);
   * 
   ...
   */
  public void refresh(GenomicRegion displayRegion, int resolution, int width, int height, int margin)
      throws IOException {
  ...
  public void update(GenomicRegion displayRegion, int resolution, int width, int height, int margin)
    ...
    for (TreeNode<Track> node : mTracks) {
      ...
      for (TreeNode<Track> child : children) {
        ...
        TrackSubFigure cf = child.getValue().updateGraph(displayRegion, resolution, width, height, margin);
    ...
    double autoNormYMax = getCommonYMax(mTracks, displayRegion, resolution, true);
   ...
   *           Signals that an I/O exception has occurred.
   */
  private static double getCommonYMax(TrackTree tracks, GenomicRegion displayRegion, int resolution, boolean normalize)
      throws IOException {
    double y = 0;
   ...
   *           Signals that an I/O exception has occurred.
   */
  private static double getYMax(TreeNode<Track> track, GenomicRegion displayRegion, int resolution, boolean normalize)
      throws IOException {
    double max = 0;
    double y;
    ...
    for (TreeNode<Track> child : children) {
      ...
      /*
       * UCSCTrack bedGraph = child.getValue().getBedGraph(displayRegion, resolution,
       * normalize);
       * 
       * // This track is not a bedgraph if (bedGraph == null) { return MIN_MAX_Y; }
       * 
       * 
       * List<UCSCTrackRegion> regions =
       * UCSCTrackRegions.getFixedGapSearch(bedGraph.getRegions()).getFeatureSet(
       * displayRegion.getChr(), displayRegion.getStart(), displayRegion.getEnd());
       * 
       * if (regions != null) { for (UCSCTrackRegion region : regions) { double value
       * = ((BedGraphRegion)region).getValue();
       * 
       * if (value > y) { y = value; } } }
       * 
       * y = Math.max(MIN_MAX_Y, y);
