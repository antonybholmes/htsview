...
public class BedPlotLayer extends AxesClippedLayer implements ModernClickListener {
	...
	/** The m dims. */
	private Map<UCSCTrackRegion, IntRect> mDims = 
			new HashMap<UCSCTrackRegion, IntRect>();
	...
	public void plotClipped(Graphics2D g2, 
		...
		for (UCSCTrackRegion region : mRegions) {
			...
			/*
			if (bed.getStrand() == '+' || bed.getStrand() == '-') {
				// draw track lower down so direction arrow can be put
				// in place
				yp = y + yDiff2;
			} else {
				// center the blocks within the space
				yp = y + yDiff1;
			}
			*/
			...
			/*
			if (bed.getStrand() == '+') {
				GenesPlotCanvasLayer.drawForwardArrow(g2, x1, yp);
			} else if (bed.getStrand() == '-') {
				GenesPlotCanvasLayer.drawReverseArrow(g2, x2, yp);
			} else {
				// do nothing
			}
			*/
