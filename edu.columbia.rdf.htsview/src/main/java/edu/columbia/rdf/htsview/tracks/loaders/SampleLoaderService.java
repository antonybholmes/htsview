...
public class SampleLoaderService {
	...
	private static class SampleLoaderLoader {
		...
		private static final SampleLoaderService INSTANCE = 
				new SampleLoaderService();
	}
	...
	/** The m parser map. */
	private Map<String, SampleLoader> mParserMap =
			new HashMap<String, SampleLoader>();
	...
	/**
	 * Open multiple sample files. This may trigger dialogs requesting
	 * user input. For unattended file opening, use either the 
	 * {@code openSample()} or {@code openReads()} methods.
	 *
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public Track openFile(ModernWindow parent,
			Path file,
			TreeNode<Track> root) throws IOException {
