...
public class ReadsPlotTrack extends SamplePlotTrack {
	...
	private static final Color DEFAULT_LINE_COLOR = 
			SettingsService.getInstance().getAsColor("edb.reads.tracks.reads-plot.strands.positive.line-color");
	
	/** The Constant DEFAULT_FILL_COLOR. */
	private static final Color DEFAULT_FILL_COLOR = 
			SettingsService.getInstance().getAsColor("edb.reads.tracks.reads-plot.strands.positive.fill-color");
	
	/** The Constant DEFAULT_NEG_STRAND_LINE_COLOR. */
	private static final Color DEFAULT_NEG_STRAND_LINE_COLOR = 
			SettingsService.getInstance().getAsColor("edb.reads.tracks.reads-plot.strands.negative.line-color");
	
	/** The Constant DEFAULT_NEG_STRAND_FILL_COLOR. */
	private static final Color DEFAULT_NEG_STRAND_FILL_COLOR = 
			SettingsService.getInstance().getAsColor("edb.reads.tracks.reads-plot.strands.negative.fill-color");

	/** The Constant MAX_READS_DISPLAY. */
	private static final int MAX_READS_DISPLAY =
			SettingsService.getInstance().getAsInt("edb.reads.tracks.reads-plot.max-display-reads");
	
	/** The Constant DEFAULT_READ_HEIGHT. */
	private static final int DEFAULT_READ_HEIGHT = 
			SettingsService.getInstance().getAsInt("edb.reads.tracks.reads-plot.default-read-height");
	
	/** The Constant DEFAULT_GAP. */
	private static final int DEFAULT_GAP = 
			SettingsService.getInstance().getAsInt("edb.reads.tracks.reads-plot.default-gap");
	 ...
	 */
	public ReadsPlotTrack(Sample sample, 
			SampleAssembly assembly) {
		this(sample, 
				assembly,
				DEFAULT_LINE_COLOR,
				DEFAULT_FILL_COLOR,
				DEFAULT_NEG_STRAND_LINE_COLOR,
	...
	public TrackSubFigure createGraph(String genome,
		...
		mSubFigure = ReadsPlotSubFigure.create(getName(),
				mReadLength,
				titlePosition);
		...
		//axes.setInternalPlotSize(Track.PLOT_WIDTH, ReadsPlotLayer.BLOCK * MAX_BLOCK_DEPTH);
	...
	/* (non-Javadoc)
	 * @see edu.columbia.rdf.apps.edb.reads.tracks.SamplePlotTrack#updateGraph(edu.columbia.rdf.lib.bioinformatics.genome.GenomicRegion, int, boolean)
	 */
	@Override
	public TrackSubFigure updateGraph(GenomicRegion displayRegion, 
		...
		List<Integer> starts = 
				mAssembly.getStarts(mSample, displayRegion, resolution);
		
		List<Strand> strands = 
				mAssembly.getStrands(mSample, displayRegion, resolution);
	...
	/* (non-Javadoc)
	 * @see edu.columbia.rdf.edb.reads.app.tracks.sample.SamplePlotTrack#edit(org.abh.common.ui.window.ModernWindow)
	 */
	@Override
	public void edit(ModernWindow parent) {
		ReadsPlotTrackEditDialog dialog = 
				new ReadsPlotTrackEditDialog(parent, this);
