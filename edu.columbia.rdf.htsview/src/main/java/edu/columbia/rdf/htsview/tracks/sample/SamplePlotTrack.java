...
public class SamplePlotTrack extends GraphPlotTrack {
	...
	private static final Color DEFAULT_COLOR = 
			SettingsService.getInstance().getAsColor("edb.reads.tracks.sample-plot.default-color");
	 ...
	 */
	public SamplePlotTrack(Sample sample, 
	 ...
	 * @param height the height
	 */
	public SamplePlotTrack(String name,
			Sample sample,
			SampleAssembly assembly,
			Color lineColor,
			Color fillColor,
	...
	public TrackSubFigure createGraph(String genome,
		...
		mSubFigure = BedGraphSubFigure.create(getName(),
				mStyle,
				titlePosition);
	...
	public TrackSubFigure updateGraph(GenomicRegion displayRegion, 
		...
		UCSCTrack bedGraph = 
				getBedGraph(displayRegion, resolution, mNormalize);
	...
	/* (non-Javadoc)
	 * @see edu.columbia.rdf.htsview.tracks.Track#getBedGraph(org.jebtk.bioinformatics.genome.GenomicRegion, int)
	 */
	@Override
	...
	public UCSCTrack getBedGraph(GenomicRegion displayRegion, 
		...
		List<Integer> counts = mAssembly.getCounts(mSample,
				displayRegion,
				resolution);

		// Subtract the input if desired
		if (mSubtract && mInputSample != null) {
			List<Integer> mInputCounts = mInputAssembly.getCounts(mInputSample,
					displayRegion,
					resolution);
	...
	/* (non-Javadoc)
	 * @see edu.columbia.rdf.htsview.tracks.GraphPlotTrack#edit(org.abh.common.ui.window.ModernWindow)
	 */
	@Override
