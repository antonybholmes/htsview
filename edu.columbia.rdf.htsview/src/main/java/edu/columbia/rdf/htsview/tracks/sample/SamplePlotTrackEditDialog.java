...
public class SamplePlotTrackEditDialog extends ModernDialogHelpWindow {
	...
	private ModernTextField mSubtractField = 
			new ModernClipboardTextField();
	
	/** The m height field. */
	private ModernCompactSpinner mHeightField =
			new ModernCompactSpinner(1, 1000, FIELD_WIDTH);
	
	/** The m check common Y. */
	private ModernRadioButton mCheckCommonY =
			new ModernRadioButton("Common Y", FIELD_WIDTH);
	
	/** The m check auto Y. */
	private ModernRadioButton mCheckAutoY =
			new ModernRadioButton("Auto Y", FIELD_WIDTH);
	
	/** The m check max Y. */
	private ModernRadioButton mCheckMaxY =
			new ModernRadioButton("Max Y", FIELD_WIDTH);
	
	/** The m check normalize Y. */
	private ModernTwoStateWidget mCheckNormalizeY =
			new ModernCheckSwitch("Normalize Y", FIELD_WIDTH);
	
	/** The m max Y field. */
	private ModernCompactSpinner mMaxYField =
			new ModernCompactSpinner(0, 10000, 1);

	/** The m track. */
	private SamplePlotTrack mTrack;

	/** The m check line color. */
	private ModernTwoStateWidget mCheckLineColor =
			new ModernCheckSwitch("Line color", FIELD_WIDTH);
	
	/** The m check fill color. */
	private ModernTwoStateWidget mCheckFillColor =
			new ModernCheckSwitch("Fill color", FIELD_WIDTH);
	
	/** The m check subtract input. */
	private ModernTwoStateWidget mCheckSubtractInput =
			new ModernCheckSwitch("Subtract", FIELD_WIDTH);
	
	/** The m check common height. */
	private ModernRadioButton mCheckCommonHeight =
			new ModernRadioButton("Common height", FIELD_WIDTH);
	
	/** The m check max height. */
	private ModernRadioButton mCheckMaxHeight =
			new ModernRadioButton("Height", FIELD_WIDTH);
	
	/** The m sample db button. */
	private ModernButton mSampleDbButton =
			new ModernDialogFlatButton(UIService.getInstance().loadIcon("database", 16));
	
	/** The m sample fs button. */
	private ModernButton mSampleFsButton =
			new ModernDialogFlatButton(UIService.getInstance().loadIcon(OpenFolderVectorIcon.class, 16));
	 ...
	 */
	public SamplePlotTrackEditDialog(ModernWindow parent, 
	...
	private void loadDbSample() throws IOException, ParseException {
		ChipSeqSamplesDialog dialog = 
				new ChipSeqSamplesDialog(mParent);
	...
	private void browseForTrack(Path pwd) throws ParseException, IOException {
		openTrack(FileDialog.open(mParent).filter(new AllSamplesGuiFileFilter(),
				new BamGuiFileFilter(),
				new Brt2GuiFileFilter(),
				new BvtGuiFileFilter()).getFile(pwd));
	}
