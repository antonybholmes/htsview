...
public class TrackView {
	...
	public static void loadJsonView(ModernWindow window,
		...
		GenomicRegion region = 
				GenomicRegion.parse(viewJson.get("location").getAsString());
		...
		if (viewJson.containsKey("titles")) {
			...
			TitleProperties titleProperties =
					new TitleProperties(position, visible);
		...
		while (!tracksQueue.isEmpty()) {
			...
			for (Json trackJson : tracksJson) {
				...
				TrackJsonParser parser = 
						TrackParserService.getInstance().get(type);
